...
const AuthModal = ({ initialMode = 'login', onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'signup' && trimmedName.length < 3) {
    ...
    try {
      if (mode === 'login') {
        ...
        if (res?.success) {
          toast.success('Verification link sent to Gmail! ✉️', { duration: 5000 });
      ...
      <div className="bg-slate-900 w-full max-w-md rounded-2xl border border-white/10 p-6 shadow-2xl">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-white">
            {isVerificationSent ? 'Verify Email' : (mode === 'login' ? 'Login Workspace' : 'Create Account')}
          </h2>
        ...
        {isVerificationSent ? (
          <div className="text-center py-4">
            <p className="text-sm text-slate-300 mb-4">
              A verification link has been sent to <span className="text-rose-400 font-semibold">{email}</span>. Please verify it before logging in.
            </p>
            <button
              onClick={() => {
              ...
              className="px-6 py-2 bg-amber-500 text-black font-bold text-xs rounded-lg uppercase tracking-wider"
            >
              Go to Login
            </button>
          </div>
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {loading ? 'Processing...' : (mode === 'login' ? 'Login' : 'Send Verification Link')}
              </button>
            </form>
          </>
        )}
